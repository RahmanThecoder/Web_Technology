...
if (!isset($_SESSION['user_login'])) {
	...
	$result = $con->query("SELECT * FROM registration WHERE id='$user'");
		$get_user_name = $result->fetch_assoc();
			...
			$pro_pic_db = $get_user_name['user_picture'];
...
if($profile_pic_name == ""){
	if($result = $con->query("UPDATE registration SET phone='$_POST[phone]', address='$_POST[address]' WHERE id='$user'")){
				$succs_message = " Dats Information Updated.";
		}
		if($utype_db == "teacher"){
				if($result = $con->query("UPDATE tutor SET inst_name='$_POST[inst_nm]' WHERE t_id='$user'")){
					$succs_message = "Data Informaion Updated!";
				}
			}
...
if (((@$_FILES['profilepic']['type']=='image/jpeg') || (@$_FILES['profilepic']['type']=='image/png') || (@$_FILES['profilepic']['type']=='image/jpg') || (@$_FILES['profilepic']['type']=='image/gif')) && (@$_FILES['profilepic']['size'] < 1000000)) {
	...
	if (file_exists("image/profilepic/".$filename)) {
		...
		if(move_uploaded_file(@$_FILES["profilepic"]["tmp_name"], "image/profilepic/".$filename)){
			$photos = $filename;
			if($result = $con->query("UPDATE registration SET phone='$_POST[phone]', address='$_POST[address]', user_pic='$photos' WHERE id='$user'")){
				$delete_file = unlink("image/profilepic/".$pro_pic_db);
				$succs_message = " Data Informaion Updated!";
			}
			if($utype_db == "teacher"){
				if($result = $con->query("UPDATE teacher SET inst_name='$_POST[inst_nm]' WHERE t_id='$user'")){
					$succs_message = " Data Informaion Updated!";
				}
			}
				header("Location: aboutme.php?uid=".$user."");
...
<div>
	<div>
		...
		<div class="topnav">
			...
			<a class="navlink" href="#news">Avaiable Tutor</a>
			<?php 
			if($utype_db == "teacher")
				{
