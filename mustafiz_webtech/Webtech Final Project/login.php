...
if (isset($_POST['login'])) {
	if (isset($_POST['email']) && isset($_POST['password'])) {
		...
		$result = $con->query("SELECT * FROM registration WHERE (email='$user_login') AND pass='$password_login_md5'");
...
if(isset($_POST['activate'])){
	if(isset($_POST['actcode'])){
		...
		$result2 = mysql_query("SELECT * FROM registration WHERE (email='$user_login') AND confirmCode='$user_acccode'");
		...
		if ($num3>0) {
			...
			$_SESSION['user_login'] = $get_user_uname_db;
			setcookie('user_login', $user_login, time() + (365 * 24 * 60 * 60), "/");
			mysql_query("UPDATE registration SET confirmCode='0', activation='yes' WHERE email='$user_login'");
...
<div>
		...
		<div class="topnav">
			...
			<a class="navlink" href="search.php">Find A Teacher</a>
			<?php 
			if($utype_db == "teacher")
				{
