...
<div>
	<div>
		...
		<div class="topnav">
			...
			<a class="active navlink" href="search.php">Avaiable Tutor</a>
			<?php 
			if($utype_db == "teacher")
				{
			...
			<?php
				if (isset($_POST['submit'])){
					...
					';
						...
						$query_sub = $con->query("SELECT * FROM `teacher` WHERE ".$condition);
						...
						<div class="p_body">
							<div class="itemrow" style="text-align: center;">
					  				<p><label>'.$query_sub_cnt.' Teacher Found </label></p>
					  		</div>
						</div>
					</div>';
					...
					while($row = $query_sub->fetch_assoc()) { 
						$post_id = $row['id'];
						$tutor_id = $row['teacher_id'];
						$sub = $row['prefer_subject'];
						$sub = str_replace(",", ", ", $sub);
						$class = $row['class'];
						...
						$p_university = $row['name_of_institution'];
						$medium = $row['medium'];

						$query1 = $con->query("SELECT * FROM registration WHERE id='$tutor_id'");
						...
						$cntnm = "";
						if($user != ""){
							$query6 = $con->query("SELECT * FROM record WHERE applied_by='$user' AND applied_to='$tutor_id'");
