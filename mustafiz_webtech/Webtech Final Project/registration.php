...
$_POST['first_name'] = trim($_POST['first_name']);
	try {
		...
		$check = 0;
		$e_check = $con->query("SELECT email FROM `registration` WHERE email='$u_email'");
			...
			if ($check == 0 ) {
				if ($email_check == 0) {
					if (strlen($_POST['password']) >1 ) {
						...
						$sql = "INSERT INTO `registration` (`fullname`,`email`,`phone`,`gender`,`pass`,`type`,`confirmcode`) VALUES ('".$u_fname."','".$u_email."','".$u_mobile."','".$u_gender."','".$u_pass."','".$u_ac."','".$confirmCode."')";
