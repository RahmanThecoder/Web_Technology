...
if (isset($_REQUEST['confirm'])) {
		...
		$result = mysqli_query($con, "INSERT INTO record (applied_by,applied_to) VALUES ('$user','$tid')");

		$result = $con->query("SELECT * FROM user WHERE id='$tid'");
		$get_user_name = $result->fetch_assoc();
			$uname = $get_user_name['fullname'];
			$utype = $get_user_name['type'];
		$error = "Contact Now!! You Successfully Selected <a href='aboutme.php?uid=".$tid."' style='color: #4CAF50;'>".$uname."</a>";
}else{
...
<div>
	<div>
		...
		<div class="topnav">
			...
			<a class="active navlink" href="index.php" style="margin: 0px 0px 0px 100px;">DashBoard Information</a>
			<a class="navlink" href="search.php">Available Tutor</a>
			<?php 
			if($utype_db == "teacher")
				{
			...
			<div style="float: right;" >
				<table>
					<tr>
						<?php
							if($user != ""){
								$resultnoti = $con->query("SELECT * FROM applied_post WHERE post_by='$user' AND student_cheaked='no'");
