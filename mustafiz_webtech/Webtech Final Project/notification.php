...
if($utype_db == "student"){
	$up = $con->query("UPDATE record SET student_cheaked='yes'");
}
if($utype_db == "teacher"){
	$up = $con->query("UPDATE record SET teacher_cheaked='yes'");
}
...
<div>
	<div>
		...
		<div class="topnav">
			...
			<a class="navlink" href="search.php">Avaiable Tutor</a>
			<?php 
			if($utype_db == "teacher")
				{
				...
				<table>
					<tr>
						<?php
							if($user != ""){
								if($utype_db == 'teacher'){
									$resultnoti = $con->query("SELECT * FROM record WHERE (applied_by='$user' AND job_confirm='1' AND teacher_cheaked='no') OR (applied_to='$user' AND teacher_cheaked='no')" );
								}else{
	...
	<div class="nbody" style="margin: 0px 100px; overflow: hidden;">
		<div class="nfeedleft">
				...
				<?php
					...
					if($utype_db == 'teacher'){
						$query = $con->query("SELECT * FROM record WHERE (applied_by='$user' AND job_confirm='1') OR (applied_to='$user') ORDER BY id DESC");
						...
						while ($row = $query->fetch_assoc()) {
							...
							$tution_confirm = $row['job_confirm'];

							if($post_by_id == 0) $post_by_id = $applied_by_id;

							$query1 = $con->query("SELECT * FROM registration WHERE id='$post_by_id'");
						...
						$query = $con->query("SELECT * FROM record WHERE post_by='$user' ORDER BY id DESC");
						while ($row = $query->fetch_assoc()) {
							$post_id = $row['post_id'];
							$applied_by_id = $row['applied_by'];
							$deadline = $row['applied_time'];
							...
							$query1 = $con->query("SELECT * FROM registration WHERE id='$applied_by_id'");
							$user_fname = $query1->fetch_assoc();
							$uname_db = $user_fname['fullname'];
							$pro_pic_db = $user_fname['user_picture'];
							$ugender_db = $user_fname['gender'];
							...
							echo '
								...
								<div style="float: right;">';
												...
												if($tution_confirm == "1"){
													echo '
												<input type="button" class="sub_button" style="margin: 0px; background-color: #a76d6d; cursor: default;" name="" value=" You Already Confirm" />';
												}else{
