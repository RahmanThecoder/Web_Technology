...
if ($senddata) {
	...
	$password_query = $con->query("SELECT * FROM registration WHERE id='$user'");
	while ($row = mysqli_fetch_assoc($password_query)) {
		...
		if ($oldpassword_md5 == $db_password) {
			if ($newpassword == $repear_password) {
				...
				$password_update_query = $con->query("UPDATE registration SET pass='$newpassword_md5', confirmcode='$confirmCode', email='$email' WHERE id='$user'");
